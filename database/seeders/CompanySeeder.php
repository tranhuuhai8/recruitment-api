...
class CompanySeeder extends Seeder
{
    ...
    {
        $remaining = self::TARGET - Company::count();
        if ($remaining <= 0) {
            return;
        }

        $demoUser = User::where('mail_address', 'company@gmail.com')->first();
        $users = collect();

        if ($demoUser && !Company::where('user_id', $demoUser->id)->exists()) {
            $users->push($demoUser);
            $remaining--;
        }

        $candidates = User::where('role', User::ROLE_COMPANY)
